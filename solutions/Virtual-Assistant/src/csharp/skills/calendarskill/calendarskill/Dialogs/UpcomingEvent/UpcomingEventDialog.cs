...
namespace CalendarSkill.Dialogs.UpcomingEvent
{
    public class UpcomingEventDialog : CalendarSkillDialog
    {
        public UpcomingEventDialog(
            ...
            : base(nameof(UpcomingEventDialog), services, accessor, serviceManager, telemetryClient)
        {
        ...
        public async Task<DialogTurnResult> ShowUpcomingMeeting(WaterfallStepContext sc, CancellationToken cancellationToken = default(CancellationToken))
        {
            try
            {
                ...
                if (nextEvent != null && (!nextEvent.IsAllDay.HasValue || nextEvent.IsAllDay == false) && timeDiff < 60)
                {
                    ...
                    {
                        Prompt = sc.Context.Activity.CreateReply(UpcomingEventResponses.UpcomingMeetingNavigateToLocationMessage, ResponseBuilder, speakParams),
                    });
                }
