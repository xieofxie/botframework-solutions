...
using CalendarSkill.Dialogs.UpcomingMeeting.Resources;
...
namespace CalendarSkill.Dialogs.UpcomingMeeting
{
    public class UpcomingMeetingDialog : CalendarSkillDialog
    {
        public UpcomingMeetingDialog(
            SkillConfiguration services,
            IStatePropertyAccessor<CalendarSkillState> accessor,
            IServiceManager serviceManager,
            IBotTelemetryClient telemetryClient)
            : base(nameof(UpcomingMeetingDialog), services, accessor, serviceManager, telemetryClient)
        {
        ...
        public async Task<DialogTurnResult> ShowUpcomingMeeting(WaterfallStepContext sc, CancellationToken cancellationToken = default(CancellationToken))
        {
            try
            {
                ...
                if (nextEvent != null && (!nextEvent.IsAllDay.HasValue || nextEvent.IsAllDay == false) && timeDiff < 60)
                {
                    ...
                    {
                        Prompt = sc.Context.Activity.CreateReply(UpcomingMeetingResponses.UpcomingMeetingNavigateToLocationMessage, ResponseBuilder, speakParams),
                    });
                }
