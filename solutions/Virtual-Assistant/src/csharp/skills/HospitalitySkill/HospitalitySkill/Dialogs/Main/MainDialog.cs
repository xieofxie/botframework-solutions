...
using HospitalitySkill.Dialogs.Shared.DialogOptions;
...
namespace HospitalitySkill.Dialogs.Main
{
    public class MainDialog : RouterDialog
    {
        ...
        protected override async Task RouteAsync(DialogContext dc, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            localeConfig.LuisServices.TryGetValue("HospitalitySkill", out var luisService);
            ...
            else
            {
                ...
                var result = await luisService.RecognizeAsync<HospitalitySkillLU>(dc.Context, CancellationToken.None);
                var intent = result?.TopIntent().intent;

                switch (intent)
                {
                    case HospitalitySkillLU.Intent.CheckIn:
                        {
                            turnResult = await dc.BeginDialogAsync(nameof(CheckInDialog), skillOptions);
                            break;
                        }

                    case HospitalitySkillLU.Intent.CheckOut:
                        {
                            turnResult = await dc.BeginDialogAsync(nameof(CheckOutDialog), skillOptions);
                            break;
                        }

                    case HospitalitySkillLU.Intent.OrderFood:
                        {
                            turnResult = await dc.BeginDialogAsync(nameof(OrderFoodDialog), skillOptions);
                            break;
                        }

                    case HospitalitySkillLU.Intent.RequestServices:
                        {
                            turnResult = await dc.BeginDialogAsync(nameof(RequestServicesDialog), skillOptions);
                            break;
                        }

                    case HospitalitySkillLU.Intent.None:
                        {
        ...
        {
            AddDialog(new CheckInDialog(_services, _responseManager, _conversationStateAccessor, _userStateAccessor, _serviceManager, TelemetryClient));
            AddDialog(new CheckOutDialog(_services, _responseManager, _conversationStateAccessor, _userStateAccessor, _serviceManager, TelemetryClient));
            AddDialog(new OrderFoodDialog(_services, _responseManager, _conversationStateAccessor, _userStateAccessor, _serviceManager, TelemetryClient));
            AddDialog(new RequestServicesDialog(_services, _responseManager, _conversationStateAccessor, _userStateAccessor, _serviceManager, TelemetryClient));
        }
